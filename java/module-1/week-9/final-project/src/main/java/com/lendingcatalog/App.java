...
public class App {
    ...
    private static final String MEMBER_FILEPATH = "members.txt";
...
// Load data from the users.dat file
        File memberFile = new File(FILE_BASE_PATH + MEMBER_FILEPATH);
        int lineCounter = 0;
        try (Scanner memberScanner = new Scanner(memberFile)) {
            while (memberScanner.hasNext()) {
                String currentLine = memberScanner.nextLine();
                lineCounter++;
                if (currentLine != null) {
                    String[] currentMember = currentLine.split(FIELD_DELIMITER);
                    if (currentMember.length == 3) {
                        Member member = new Member(currentMember[0], currentMember[2]);
                        File items = new File(FILE_BASE_PATH + currentMember[2]);
                        try (Scanner itemScanner = new Scanner(items)) {
                            int itemLineCounter = 0;
                            List<CatalogItem> memberItems = new ArrayList<>();
                            while (itemScanner.hasNextLine()) {
                                String currentItemLine = itemScanner.nextLine();
                                itemLineCounter++;
                                if (currentItemLine != null) {
                                    String[] currentItem = currentItemLine.split(FIELD_DELIMITER);
                                    if (currentItem.length == 4) {
                                        if (currentItem[0].contains("book")) {
                                            Book book = new Book(currentItem[1], currentItem[2], LocalDate.parse(currentItem[3]));
                                            book.registerItem();
                                            memberItems.add(book);
                                        } else if (currentItem[0].contains("movie")) {
                                            Movie movie = new Movie(currentItem[1], currentItem[2], LocalDate.parse(currentItem[3]));
                                            memberItems.add(movie);
                                        } else if (currentItem[0].contains("tool")) {
                                            Tool tool = new Tool(currentItem[1], currentItem[2], Integer.parseInt(currentItem[3]));
                                            memberItems.add(tool);
                                        }
                                    } else {
                                        System.out.println("Review file, line " + itemLineCounter + " is invalid");
                                    }
                                } else {
                                    System.out.println("Review file, line " + itemLineCounter + " is invalid");
                                }
                            }
                            catalog.put(member.toString(), memberItems);
                        } catch (FileNotFoundException e) {
                            // Handle the exception appropriately.
                            e.printStackTrace();
                        }
                    } else {
                        System.out.println("Review file, line " + lineCounter + " is invalid");
                    }
                } else {
                    System.out.println("Review file, line " + lineCounter + " is invalid");
                }
