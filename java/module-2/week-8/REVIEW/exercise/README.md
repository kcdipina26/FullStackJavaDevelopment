...
> Note: The collection contains login requests for `jessa` and `admin`. If you want to test other users, you can create requests to login. Modify the data in the `Body` of the request. Other user names are `liam`, `antoni`, `sofia`, `mark`, and `susan`. For all these users, the password is `password`.
...
On `TodoController`, add an annotation to ensure that before any user can call any of its methods, that call must include an authentication token (that is, the user must be authenticated).
...
Best practice is to do any list filtering in the DAO. If the data store is a SQL database, you'll use a `WHERE` clause to prevent transmitting too many rows across the network. In this application, the data store is an in-memory array, but you'll still do the work in the DAO.
...
> **Hint:** One way to avoid code duplication is to first call the `get()` method from within `update()`. If the todo s not found, or if the user doesn't have access to the todo, then `get()` throws the appropriate exception (`404` or `403`). If `get()` returns without an exception, then the update may proceed.
